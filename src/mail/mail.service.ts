...
export class MailService {
  ...
  constructor() {
    ...
    if (!gmailUser || !clientId || !clientSecret || !refreshToken) {
      this.logger.error(
        'Missing Gmail API env vars: GMAIL_USER / GMAIL_CLIENT_ID / GMAIL_CLIENT_SECRET / GMAIL_REFRESH_TOKEN',
      );
    }
    ...
    const oAuth2Client = new google.auth.OAuth2(
      clientId,
      clientSecret,
      // redirect URI không dùng nữa ở đây, nhưng phải trùng với lúc anh tạo token
  ...
  async sendOtpEmail(email: string, otp: string): Promise<boolean> {
    try {
      ...
      const subject = 'Your OTP Code for Zalo Like';
      ...
      this.logger.log(`Email sent successfully to ${email} via Gmail API`);
      return true;
    } catch (error) {
      this.logger.error(
